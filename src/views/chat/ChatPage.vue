...
// 这是保存用户信息列表的
import {  updatefriend ,logoutuser,getuserInfo } from '@/api/apiuser.js'
...
const clickPerson = async (info) => {
    ...
    for(let i = 0; i < personList.value.length; ++i){
        if(info.id == personList.value[i].id) cc = personList.value[i]
    }
    personInfo.value = cc
    chatWindowInfo.value = cc
