...
import { ref , watch, onMounted, defineProps, defineExpose, defineEmits, useSSRContext,nextTick} from 'vue'
...
import { deletefriend , addfriend, searchfriends,  updatefriend,findFriend } from '@/api/apiuser'
...
const change = async (id)=>{
  ...
  if(id === 1){
    ...
    console.log('用户的名称', userid.value);
    myid = userid.value
...
const clickPerson = (info) => {
  console.log(info);
    if(nowvalue.value === 3){
      ...
      msgCurrentFriendId.value = info.friendId
    }
...
const searchfriend = async ()=>{
  ...
  if(searchvalue.value.trim == ''){
    ...
    if(nowvalue.value === 2){
      ...
      const id = searchvalue.value
      const obj = {
        id: id
      }
      const friendInfo = await findFriend(obj)
      ...
      username.value = friendInfo.name
      userdetail.value = friendInfo.detail
      userid.value =  friendInfo.id
      userimg.value =  friendInfo.headImg
...
const closeinner = async (id)=>{
  ...
  if(id === 1){
    ...
    if(nowvalue.value === 3){
      ...
      //     infos = Messagelist.value[i]
      //   }
      // }
      let info = {}
      info.id = msgCurrentFriendId.value
